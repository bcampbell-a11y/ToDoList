...
exports.handler = async function(event) {
  ...
  // Get GitHub token from env or request header fallback
  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (!sectionId) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: `Unknown section: "${section}"` }) };
      }
 
      const data = await getGist();
      if (!data.sections || !data.sections[sectionId]) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: `Section not found: ${sectionId}` }) };
      }
      ...
      if (action === 'complete') {
        const idx = taskIndex !== undefined ? taskIndex : data.sections[sectionId].tasks.findIndex(function(t) { return t.text.toLowerCase().indexOf((task || '').toLowerCase()) !== -1; });
